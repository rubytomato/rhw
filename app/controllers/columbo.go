...
func (c Columbo) Result(s forms.SearchForm) revel.Result {
	...
	for _, v := range episodes {
		...
		if s.Title != "" {
/*
			vv := strings.ToLower(v.Title)
			ss := strings.ToLower(s.Title)
			if !strings.Contains(vv, ss) {
				unmatch = true
			}
*/
...
/*
			vv := strings.ToLower(v.OriginalAirDate)
			ss := strings.ToLower(s.OriginalAirDate)
			if !strings.Contains(vv, ss) {
				unmatch = true
			}
*/
		...
		if s.GuestStaring != "" {
/*
			vv := strings.ToLower(v.GuestStaring)
			ss := strings.ToLower(s.GuestStaring)
			if !strings.Contains(vv, ss) {
				unmatch = true
			}
*/
